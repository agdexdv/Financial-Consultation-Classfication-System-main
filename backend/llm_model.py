"""
LLM 模型推理模块
基于 Qwen2.5-3B-Instruct + LoRA 进行金融文本分类
"""
import os
import torch
...
class FinancialLLMClassifier:
    ...
    def _load_model(self):
        """加载模型和 tokenizer"""
        # 暂时禁用 LLM 模型 - 需要 12GB+ GPU 内存才能稳定运行
        # 当前 TensorFlow 占用部分 GPU 内存，无法同时加载 Qwen2.5-3B
        # 模型文件已下载，待升级 GPU 内存或移除 TensorFlow 后可启用
        self.load_error = (
            "llm_disabled: LLM 模型已禁用（需要 12GB+ GPU 内存）。\n"
